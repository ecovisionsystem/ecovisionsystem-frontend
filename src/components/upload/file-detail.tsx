  ...
  const groups = [
    ...
    },
    {
      ...
      items: [
        ...
        ["S3 Upload", file.uploadId ? "Presigned PUT / Verified" : "Pending"],
        ["HPC Target", "Keele SLURM"],
        ["Model", "SegFormer-B5 + ConvNeXt"],
      ],
    },
  ];
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          {[
            ...
            ["Classification", "ConvNeXt-Base"],
            ["Uncertainty", "MAPIE Conformal"],
            ["Hardware", "EC2 G4dn Spot"],
          ].map(([key, value]) => (
            <div
              key={key}
      ...
      <div>
        ...
        <div className="space-y-1">
          {stages.map(([label, detail], index) => {
            const done = step > index || (!running && isUploaded);
...
function ResultsTab({ file }: { file: UploadQueueFile }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <SectionLabel>Species Composition</SectionLabel>
      ...
      <div>
        <SectionLabel>Ecological Metrics</SectionLabel>
        <div className="grid grid-cols-2 gap-2">
          {[
            ["Dominance Index", "0.72", T.lime],
            ["mIoU", "0.941", T.leaf],
            ["Pixel Accuracy", "96.2%", T.lime],
            ["Blobs Detected", "214", T.leaf],
          ].map(([key, value, color]) => (
            <div
              key={key}
            ...
            >
              ...
              <div className="text-2xl" style={{ fontFamily: T.serif, color }}>
                {value}
              </div>
            </div>
          ))}
      ...
      <div>
        ...
        <div className="space-y-2">
          {[
            ["GeoJSON - Species polygons", "WGS84 / all classes"],
            ["Segmentation mask (GeoTIFF)", "Class-indexed / 8-bit"],
            ["Dominance score table (CSV)", "Per-polygon / UTF-8"],
            ["Inference report (PDF)", "Methods + metrics"],
          ].map(([label, sub]) => (
            <button
              key={label}
              type="button"
              className="flex w-full items-center gap-3 rounded-[10px] border px-4 py-3 text-left transition-colors hover:bg-white"
              style={{ background: T.paper, borderColor: T.border }}
            >
...
function UploadMetadataComparison({
  ...
  return (
    <div>
      <SectionLabel>Upload Metadata Comparison</SectionLabel>
      <div
  ...
  return (
    <div className="space-y-5">
      <div
        ...
        style={{ background: T.paper, borderColor: T.border }}
      >
        <SectionLabel>Upload Progress Comparison</SectionLabel>
      ...
      <DetailGroup
        title="Upload Registration Comparison"
        items={[
          ["Job ID", file.jobId || "-"],
          ["Upload ID", file.uploadId || "-"],
          ["S3 upload URL", file.uploadUrl || "-"],
    ...
    <div className="space-y-5">
      {file.status === "uploaded" ? (
        ...
        <div className="flex flex-col items-center justify-center gap-3 rounded-xl border py-10 text-center">
          ...
          <div className="text-sm leading-relaxed" style={{ color: T.muted }}>
            The S3 object reference will appear
            <br />
      ...
      <DetailGroup
        title="S3 Object Comparison"
        items={[
          ["Bucket", file.s3Bucket || "-"],
          ["Key", file.s3Key || "-"],
          ["Object ref", file.objectRef || "-"],
          ["Job ID", file.jobId || "-"],
          ["Upload ID", file.uploadId || "-"],
        ]}
      />
    ...
    <aside className="flex min-h-[640px] flex-col items-center justify-center gap-4 bg-white p-10 text-center">
      ...
      <div>
        ...
        <div className="text-sm leading-relaxed" style={{ color: T.muted }}>
          ...
          <br />
          to view details and run inference.
        </div>
      </div>
    </aside>
  );
